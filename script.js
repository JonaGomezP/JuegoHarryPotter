window.onload = () => {
    ...
    abajo.onclick = () => {
        ...
        let timer = setTimeout(moverMalo,1000);
    }
    ...
    derecha.onclick = () => {
        ...
        //Cuando se mueve al prota, se mueve automáticamente al malo
        let timer = setTimeout(moverMalo(this.posicion_prota),1000);
    }
}

//Función que mueve al malo
function moverMalo(pos_prota) {
    console.log(pos_prota)
    let celdas = Array.from(document.getElementsByTagName("td"));
    celdas.forEach(e =>{
        if(e.id == "malo"){
            let posicion_actual_malo = celdas.indexOf(e);
        }
    })
